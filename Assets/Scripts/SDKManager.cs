...
{
    private string loggerUrl = "git@github.com:pinkfong-tech/com.thepinkfongcompany.event.logger.git"; // Logger 전용 Git URL
    private string loggerPath = "MyPackages/EventLogger"; // Logger 클론 경로 (유니티 프로젝트 상대 경로)
    private bool isLoggerImported = false; // Logger 임포트 여부 확인
    ...
    private void OnGUI()
    {
        ...
        // Logger 임포트 여부 갱신
        isLoggerImported = Directory.Exists(Path.Combine(Application.dataPath, loggerPath));

        // Logger 다운로드 및 임포트 버튼
        GUI.enabled = !isLoggerImported; // 이미 임포트된 경우 버튼 비활성화
        if (GUILayout.Button("Download and Import Logger"))
        {
            CloneAndImportPackage(loggerUrl, loggerPath);
        }

        // Logger 삭제 버튼
        GUI.enabled = isLoggerImported; // 임포트된 경우에만 버튼 활성화
        if (GUILayout.Button("Delete Logger"))
        {
            DeletePackage(loggerPath);
        }
